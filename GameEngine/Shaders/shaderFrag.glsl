...
// Precomputed disk offsets for cubemap PCF
...
float CalcDirShadow() {
    ...
    // Flip normal based on face orientation for bias too
...
// Ambient + diffuse + spec for a single light
vec3 CalcSingleLight(Light light, vec3 L, float shadowFactor) {
    // flip normal if we're on a back face
    vec3 N = normalize(Normal) * (gl_FrontFacing ? 1.0 : -1.0);
...
void main() {
    ...
    // final
    result *= tex.a;
    color = vec4(tex.rgb * result, tex.a);
}
